...
internal static class CardSizer
{
    ...
    private const double CardPadX = 32, CardPadY = 28, CardMinW = 180, CardMaxW = 320;
    ...
    private static Size Card(NodeModel node, ITextMeasurer m)
    {
        ...
        double width = CardWidth(node, m, iconBlock);
    ...
    /// <see cref="CardMinW"/>); otherwise the card auto-grows to hold its widest text row on one line,
    /// clamped to [<see cref="CardMinW"/>, <see cref="CardMaxW"/>]. Past the cap, text wraps.
    /// </summary>
    private static double CardWidth(NodeModel node, ITextMeasurer m, double iconBlock)
    {
        if (node.Width is double authored) return Math.Max(CardMinW, authored);
        double chrome = CardPadX + Border + iconBlock;
        ...
        return Math.Clamp(natural, CardMinW, CardMaxW);
    }

    /// <summary>The horizontal space a card's text column wraps within — matches <see cref="Card"/>.</summary>
    internal static double CardTextAvail(NodeModel node, ITextMeasurer m)
    {
        double iconBlock = Icons.ResolveIcon(node.Icon) != null ? IconW + IconGap : 0;
        return CardWidth(node, m, iconBlock) - CardPadX - Border - iconBlock;
    }
