...
| `src/route/orthogonal.ts` | `Route/OrthogonalRouter.cs` | `autoSides` (rank-aware, `SAME_RANK_EPS 6`), `sidesFor` feedback diversion (`againstFlow` + `spansObstacle` → same-face U-loop), `laneDetour` (`CHANNEL_OFFSET 18`, `LANE_PAD 22`, `LANE_MARGIN 6`), fan-channel stagger (C# addition, diverges from TS: the step turn column starts at the gap midpoint then shifts by `|toShift| − |fromShift|` toward the fanning node, clamped `CHANNEL_OFFSET` off both faces — outer fan edges turn earliest, so parallel runs get parallel channels instead of one merged trunk), self-loops (extent 30, face = right for TB/BT, bottom for LR/RL), `segHitsRect` inset 3, `sCurve` 0.4-offset cubic. |
