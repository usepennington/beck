...
public sealed class CardSizeParityTests
{
    ...
    private static readonly string _golden = Path.Combine(AppContext.BaseDirectory, "Goldens", "measure", "cards.json");
    ...
    public void CardSizer_MatchesBrowser()
    {
        var goldens = JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, Wh>>>(
            File.ReadAllText(_golden), new JsonSerializerOptions { PropertyNameCaseInsensitive = true })!;
        ...
        foreach (var (file, nodes) in goldens)
        {
            ...
            {
                var got = CardSizer.Measure(byId[id], measurer);
