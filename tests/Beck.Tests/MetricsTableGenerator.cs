...
public class MetricsTableGenerator
{
    ...
    private static readonly int[] Extras =
    [
    ...
    private static readonly int[] SansWeights = [400, 500, 600, 700];
    private static readonly int[] MonoWeights = [400, 500, 700];
    ...
    public void Generate()
    {
        ...
        var sansAscent = new double[SansWeights.Length];
        var sansDescent = new double[SansWeights.Length];
        var sansFallback = new double[SansWeights.Length];
        var ascii = new double[SansWeights.Length][];              // [weight][cp-32]
        var extra = new Dictionary<int, double[]>();              // cp -> [weight] (only glyphs present in the font)

        for (int wi = 0; wi < SansWeights.Length; wi++)
        {
            using var f = Loaded.Open(Font(mono: false, SansWeights[wi]));
            ...
            foreach (int cp in Extras)
            {
                ...
                if (!extra.TryGetValue(cp, out var row)) extra[cp] = row = new double[SansWeights.Length];
                row[wi] = adv.Value;
            }
        }

        Arr(sb, "SansWeights", SansWeights);
        ...
        var monoAscent = new double[MonoWeights.Length];
        var monoDescent = new double[MonoWeights.Length];
        var monoAdvance = new double[MonoWeights.Length];
        for (int wi = 0; wi < MonoWeights.Length; wi++)
        {
            using var f = Loaded.Open(Font(mono: true, MonoWeights[wi]));
            (monoAscent[wi], monoDescent[wi]) = f.VMetrics();
            monoAdvance[wi] = f.Advance('x') ?? 0.6; // fixed-pitch: every glyph shares this advance
        }
        Arr(sb, "MonoWeights", MonoWeights);
