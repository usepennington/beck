...
internal static class Coerce
{
    ...
    {
        if (v is null) return EmptyMap;
        if (v is IReadOnlyDictionary<string, object?> d) return d;
        throw new BeckYamlException($"`{field}` must be a mapping");
    }

    public static IReadOnlyList<object?> AsArray(object? v, string field)
    {
        if (v is null) return EmptyList;
        if (v is IReadOnlyList<object?> l) return l;
    ...
    public static string AsString(object? v, string field)
    {
        ...
        if (v is double or bool) return Js.Str(v);
    ...
    public static double? OptNumber(object? v, string field)
    {
        ...
        if (v is string s && Js.TryNumber(s, out double n)) return n;
        ...
        where TEnum : struct, Enum
    {
        if (v is null) return dflt;
        string s = Js.Str(v);
        if (map.TryParse(s, out TEnum e)) return e;
