...
internal sealed class SequencePainter
{
    ...
    public string Render(DiagramModel model, SequenceLayoutResult layout, bool motion = false)
    {
        ...
        var bandSpec = _style.Typography.Roles.Of(FontRole.BandLabel);
        var msgSpec = _style.Typography.Roles.Of(FontRole.MsgText);

        // ---- section bands (behind everything) ----
        for (var i = 0; i < layout.Bands.Count; i++)
        {
            var b = layout.Bands[i];
            ...
            var lw = _m.Measure(b.Label, FontRole.BandLabel, bandSpec).Width;
        ...
        // ---- lifelines ----
        var cardBottom = model.Nodes.Min(p => layout.Nodes[p.Id].Y + layout.Nodes[p.Id].H);
        var seqEdges = _style.Edges;
        ...
        var palette = seqEdges.BaseColorPalette.Count > 0;
        var partIndex = new Dictionary<string, int>();
        for (var i = 0; i < model.Nodes.Count; i++)
        {
            partIndex[model.Nodes[i].Id] = i;
        }

        var partById = model.Nodes.ToDictionary(n => n.Id);
        var sharedStroke = palette ? "" : Lifeline(Defaults.EdgeColor, cardBottom, layout.LifelineBottom);
        for (var pi = 0; pi < model.Nodes.Count; pi++)
        {
            var p = model.Nodes[pi];
            var cx = layout.Centers[p.Id];
            var card = layout.Nodes[p.Id];
            ...
            var les = _style.Edges;
            var bedColor = les.UnderlayColor.Length > 0 ? les.UnderlayColor : "var(--beck-edge-underlay, var(--beck-edge))";
            ...
            {
                var llD = Shaping.BowLine(cx, y1, cx, y2, Math.Max(_style.Edges.BowAmplitude, 2), _hash + ":ll:" + p.Id);
        ...
        // ---- activation bars ----
        for (var bi = 0; bi < layout.Activations.Count; bi++)
        {
            var b = layout.Activations[bi];
            var cx = layout.Centers[b.Participant];
            var x = cx - SequenceLayout.BarHalf + b.Level * SequenceLayout.LevelStep;
            var h = Math.Max(SequenceLayout.BarHalf * 2, b.Y2 - b.Y1);
            ...
            var actAccent = SvgWriter.Attr(b.Accent);
        ...
        {
            var edge = model.Edges[row.Index];
            ...
            var msgColor = palette ? SourceHue(edge, seqEdges, partIndex, partById) : edge.Color;
            ...
            if (row.Self)
            {
                var off = SequenceLayout.ActivationOffset(layout.Activations, edge.From, row.Y);
                var sx = cxFrom + off;
                ...
                var selfD = StepRound.RoundedPath(poly, 9);
            ...
            {
                double dir = Math.Sign(cxTo - cxFrom); if (dir == 0)
                {
                    dir = 1;
                }

                var x1 = cxFrom + dir * SequenceLayout.ActivationOffset(layout.Activations, edge.From, row.Y);
                var x2 = cxTo - dir * SequenceLayout.ActivationOffset(layout.Activations, edge.To, row.Y);
                var msgD0 = $"M {N(x1)} {N(row.Y)} L {N(x2)} {N(row.Y)}";
                // Bow the straight message when the style opts in (endpoints preserved); classic → verbatim.
                var msgD = Shaping.EdgePath(_style, msgD0,
                    [new Point(x1, row.Y), new Point(x2, row.Y)], _hash + ":" + edge.Id);
                ...
                if (!string.IsNullOrEmpty(edge.Label))
                {
                    var mw = _m.Measure(edge.Label!, FontRole.MsgText, msgSpec).Width;
    ...
    private string MsgPath(EdgeModel edge, string d, string strokeColor)
    {
        ...
        {
            sb.Append($" stroke-dasharray=\"{_style.Strokes.EdgeDash}\"");
        }
        ...
        var markerColor = es.MarkerColor is { } mkc && edge.Color == Defaults.EdgeColor ? mkc : strokeColor;
        if ((edge.MarkerEnd ?? (edge.Arrow is ArrowEnds.End or ArrowEnds.Both ? MarkerShape.Arrow : null)) is { } m)
        {
    ...
    {
        if (!_motion || _style.Edges.Overlay == EdgeOverlay.None)
        {
            return "";
        }
        ...
        double padX, double padY, string fontStyle, double size, int weight, string? extraStyle)
    {
        ...
        var ls = extraStyle != null ? ";" + extraStyle : "";
    ...
    {
        if (_fades.TryGetValue(color, out var hit))
        {
            return hit;
        }

        var id = $"beck-fade-{_gradSeq++}-{_hash}";
    ...
    {
        if (edge.ColorAuthored)
        {
            return edge.Color;
        }

        var src = partById[edge.From];
        return IsDefaultAccent(src) ? es.PaletteHue(partIndex[edge.From])! : src.Accent;
    }

    private string ActivationFill(string accent)
    {
        if (_actFills.TryGetValue(accent, out var hit))
        {
            return hit;
        }

        var id = $"beck-act-{_gradSeq++}-{_hash}";
