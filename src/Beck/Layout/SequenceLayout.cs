...
{
    private static readonly Size _fallback = new(160, 56);
    ...
    public static SequenceLayoutResult Compute(DiagramModel model, IReadOnlyDictionary<string, Size> sizes)
    {
        ...
        Size Size(string id) => sizes.GetValueOrDefault(id, _fallback);
        var gap = Math.Max(48, model.Meta.Spacing.Node);
        ...
        for (var i = 0; i < gapNeed.Length; i++)
        {
            gapNeed[i] = gap;
        }
        ...
        foreach (var e in model.Edges)
        {
            ...
            if (e.From == e.To)
            {
                var need = SelfLoop + 10 + LabelEst(e.Label);
                if (a < gapNeed.Length)
                {
                    gapNeed[a] = Math.Max(gapNeed[a], need);
                }
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            {
                x += Size(parts[i - 1].Id).W / 2 + gapNeed[i - 1] + w / 2;
            }

            centers[parts[i].Id] = x;
        }

        var maxCardH = Math.Max(parts.Count > 0 ? parts.Max(p => Size(p.Id).H) : 0, _fallback.H);
        var rows = new List<MessageRow>();
        var bands = new List<SectionBand>();
        var bottom = CanvasPad + maxCardH + HeadGap;
        ...
        for (var i = 0; i < model.Edges.Count; i++)
        {
            ...
            var self = model.Edges[i].From == model.Edges[i].To;
            var y = bottom + LabelRoom;
        ...
        CloseBand();
        var lifelineBottom = bottom + Tail;
        ...
        double width = 0;
        foreach (var p in parts) { var r = nodes[p.Id]; width = Math.Max(width, r.X + r.W); }
        var last = parts[^1];
        foreach (var e in model.Edges)
        {
        ...
        var height = Math.Ceiling(lifelineBottom + CanvasPad);
        ...
        IReadOnlyList<EdgeModel> edges, List<MessageRow> rows, Dictionary<string, string> accentOf)
    {
        ...
        for (var i = 0; i < edges.Count; i++) { lastTouch[edges[i].From] = i; lastTouch[edges[i].To] = i; }

        var openBars = new List<(string Participant, int Start, int End, int Level)>();
        for (var i = 0; i < edges.Count; i++)
        {
            var e = edges[i];
            if (e.Reply || e.From == e.To)
            {
                continue;
            }
            ...
            var level = openBars.Count(b => b.Participant == e.To && b.Start <= i && i <= b.End);
    ...
    public static double ActivationOffset(IReadOnlyList<ActivationBar> bars, string participant, double y)
    {
        ...
        {
            if (b.Participant == participant && b.Y1 <= y && y <= b.Y2)
            {
                depth = Math.Max(depth, b.Level + 1);
            }
        }
