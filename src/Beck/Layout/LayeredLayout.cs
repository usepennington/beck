...
{
    private static readonly Size Fallback = new(180, 64);
    ...
    public static LayoutResult Compute(DiagramModel model, IReadOnlyDictionary<string, Size> sizes)
    {
        ...
        double gap = model.Meta.Spacing.Node;
        double rankGap = model.Meta.Spacing.Rank;

        var nodeById = model.Nodes.ToDictionary(n => n.Id);
        var groupById = model.Groups.ToDictionary(g => g.Id);
        Size SizeOf(string id) => sizes.GetValueOrDefault(id, Fallback);
        ...
        List<string> DirectChildren(string? containerId)
        {
            if (containerId is null)
            {
                ...
                foreach (var n in model.Nodes) if (ParentOf(n.Id) is null) kids.Add(n.Id);
                foreach (var g in model.Groups) if (ParentOf(g.Id) is null) kids.Add(g.Id);
                return kids;
            }
        ...
        string? RepIn(string x, string? containerId)
        {
            ...
            {
                string? p = ParentOf(cur);
                if (p is null || ++guard > 10000) return null;
                cur = p;
            }
            return cur;
        }
        ...
        List<(string, string)> ProjectedEdges(string? containerId, HashSet<string> childSet)
        {
            ...
            {
                string? cu = RepIn(e.From, containerId);
                string? cv = RepIn(e.To, containerId);
                if (cu is null || cv is null || cu == cv) continue;
                if (!childSet.Contains(cu) || !childSet.Contains(cv)) continue;
                string key = cu + " " + cv;
        ...
        Composed LayoutContainer(string? containerId)
        {
            ...
            {
                if (visiting.Contains(containerId)) return new Composed(new(), new(), 0, 0);
            ...
            foreach (var c in children)
            {
                ...
                {
                    Composed sub = LayoutContainer(c);
                    if (sub.NodeRects.Count == 0) continue; // skip empty groups
                    subs[c] = sub;
            ...
            {
                if (!layer.Rects.TryGetValue(c, out var lr)) continue;
                Rect r = lr.Offset(padL, padT);
                if (groupById.ContainsKey(c))
                {
                    Composed sub = subs[c];
                    foreach (var (nid, nr) in sub.NodeRects) nodeRects[nid] = nr.Offset(r.X, r.Y);
                    foreach (var (gid, gr) in sub.GroupRects) groupRects[gid] = gr.Offset(r.X, r.Y);
                    groupRects[c] = r;
                }
                else nodeRects[c] = r;
            }
        ...
        Composed root = LayoutContainer(null);
        var nodes = new Dictionary<string, Rect>();
        var groups = new Dictionary<string, Rect>();
        foreach (var (id, r) in root.NodeRects) nodes[id] = r.Offset(CanvasPad, CanvasPad);
        foreach (var (id, r) in root.GroupRects) groups[id] = r.Offset(CanvasPad, CanvasPad);

        double width = Math.Ceiling(root.Width + CanvasPad * 2);
        double height = Math.Ceiling(root.Height + CanvasPad * 2);

        double gutter = BackEdgeGutter(model, nodes);
        if (gutter > 0)
        {
            bool horizontalSecondary = dir is Direction.TB or Direction.BT;
            double dx = horizontalSecondary ? gutter : 0;
            double dy = horizontalSecondary ? 0 : gutter;
            foreach (var id in nodes.Keys.ToList()) nodes[id] = nodes[id].Offset(dx, dy);
            foreach (var id in groups.Keys.ToList()) groups[id] = groups[id].Offset(dx, dy);
            if (horizontalSecondary) width += gutter * 2; else height += gutter * 2;
        }
    ...
    private static double BackEdgeGutter(DiagramModel model, Dictionary<string, Rect> nodes)
    {
        ...
        foreach (var e in model.Edges)
        {
            if (!nodes.TryGetValue(e.From, out var f) || !nodes.TryGetValue(e.To, out var t)) continue;
            if (e.From == e.To)
            {
            ...
            if (!Geometry.AgainstFlow(f, t, dir)) continue;
            double labelExtent = !string.IsNullOrEmpty(e.Label) ? (horizontalSecondary ? e.Label.Length * 7 + 8 : 14) : 0;
            double want = LaneReserve + (labelExtent > 0 ? LabelReserveGap + labelExtent : LaneReserve);
    ...
    {
        bool horizontal = dir is Direction.LR or Direction.RL;
        var nodeIds = items.Select(i => i.Id).ToList();
        var itemMap = items.ToDictionary(i => i.Id);
        var idIndex = new Dictionary<string, int>();
        for (int i = 0; i < nodeIds.Count; i++) idIndex[nodeIds[i]] = i;

        Size SizeOf(string id) => itemMap.TryGetValue(id, out var it) ? new Size(it.W, it.H) : Fallback;
        ...
        foreach (var (f, t) in edges)
            if (f != t && idIndex.ContainsKey(f) && idIndex.ContainsKey(t)) expanded.Add((f, t));

        // ---- cycle break: mark back edges via iterative DFS (3-color) ----
        var adj = new Dictionary<string, List<string>>();
        foreach (var id in nodeIds) adj[id] = new();
        foreach (var (f, t) in expanded) adj[f].Add(t);
        var color = new Dictionary<string, int>();
        foreach (var id in nodeIds) color[id] = 0;
        ...
        foreach (var id in nodeIds) rank[id] = 0;
        {
        ...
        foreach (var it in items) if (it.Rank is double rr) rank[it.Id] = rr;

        var distinct = nodeIds.Select(id => rank[id]).Distinct().OrderBy(x => x).ToList();
        var compress = new Dictionary<double, int>();
        for (int i = 0; i < distinct.Count; i++) compress[distinct[i]] = i;
        foreach (var id in nodeIds) rank[id] = compress[rank[id]];
        int maxRank = distinct.Count - 1;

        // ---- virtual nodes on long edges + adjacency ----
        var order = new List<List<string>>();
        for (int r = 0; r <= maxRank; r++) order.Add(new());
        foreach (var id in nodeIds) order[(int)rank[id]].Add(id);
        ...
        Dictionary<string, int> OrderIndex(int r)
        {
            ...
            for (int i = 0; i < order[r].Count; i++) m[order[r][i]] = i;
            return m;
        }
        double BaryOf(string id, Dictionary<string, int> nri, bool useDown)
        {
            var neigh = (useDown ? down : up).GetValueOrDefault(id);
            if (neigh is null || neigh.Count == 0) return -1;
            double sum = 0; int count = 0;
            foreach (var n in neigh) if (nri.TryGetValue(n, out int pos)) { sum += pos; count++; }
            return count > 0 ? sum / count : -1;
        }
        double TieOf(string id)
        {
            if (idIndex.TryGetValue(id, out int idx)) return idx;
            string t = id.Trim();
            return t.Length > 1 && double.TryParse(t.AsSpan(1), System.Globalization.CultureInfo.InvariantCulture, out double n) ? n : 0;
        }
        void ReorderRank(int r, bool adjacentIsDown)
        {
            ...
            {
                double b = BaryOf(id, adjIndex, adjacentIsDown);
                return (Id: id, Key: b < 0 ? (double)i : b, Tie: TieOf(id));
            });
        ...
        {
            if (sweep % 2 == 0) for (int r = 1; r <= maxRank; r++) ReorderRank(r, false);
            else for (int r = maxRank - 1; r >= 0; r--) ReorderRank(r, true);
        }

        // honor explicit item.order as a final per-rank sort
        for (int r = 0; r <= maxRank; r++)
        {
            if (!order[r].Any(id => itemMap.GetValueOrDefault(id)?.Order is not null)) continue;
        ...
        for (int r = 0; r <= maxRank; r++)
        {
            ...
            foreach (var id in row) { double b = BreadthAny(id); centers.Add(cursor + b / 2); cursor += b + gap; }
            double total = cursor - gap;
            double shift = -total / 2;
        ...
        {
            double d = 0;
            foreach (var id in order[r]) d = Math.Max(d, DepthAny(id));
            rankDepth[r] = d != 0 ? d : Fallback.H;
        }
        var primaryStart = new double[maxRank + 1];
        double acc = 0;
        for (int r = 0; r <= maxRank; r++) { primaryStart[r] = acc; acc += rankDepth[r] + rankGap; }
        double totalPrimary = acc - rankGap;
        ...
        (double X, double Y) CenterXY(string id)
        {
            ...
            if (!horizontal) return (s, dir == Direction.BT ? totalPrimary - p : p);
        ...
        if (rects.Count == 0) return new LayerResult(rects, 0, 0);

        double minX = rects.Values.Min(r => r.X), minY = rects.Values.Min(r => r.Y);
        foreach (var id in rects.Keys.ToList()) rects[id] = rects[id].Offset(-minX, -minY);
        double width = rects.Values.Max(r => r.X + r.W);
        double height = rects.Values.Max(r => r.Y + r.H);
