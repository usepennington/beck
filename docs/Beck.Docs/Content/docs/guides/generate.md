...
> [!TIP]
> `Beck.Sample` is a runnable end-to-end example of generating diagrams from code. Run `dotnet run --project src/Beck.Sample -c Release` to print one to stdout — pass `sequence`, `state`, `class`, or `reflection` to see the other builders in use.
